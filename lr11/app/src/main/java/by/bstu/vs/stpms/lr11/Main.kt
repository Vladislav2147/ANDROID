...
//    nullableInt = readLine()?.toIntOrNull()
//    println(nullableInt)
...
//    println("ababab".coincedense("ab"))
    ...
    var hasBadMarks = map.any { it.value < 4 }
...
fun String.coincedense(substring: String) = Regex(substring).findAll(this).map { it.range.first }.toList().size